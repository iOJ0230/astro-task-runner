...
class TaskRouteTest {
    ...
    @Test
    fun `should create dark-window task and return Task with id`() = testApplication {
        application {
            module()
        }

        val response = client.post("/api/tasks/dark-window") {
    ...
    @Test
    fun `should run dark-window task executes and return outputJson`() = testApplication {
        application {
            module()
        }

        // 1. create a task
        val createResponse = client.post("/api/tasks/dark-window") {
            contentType(ContentType.Application.Json)
            setBody(
                """
                {
        ...
        // 2. run the task
        val runResponse = client.post("/api/tasks/${createdTask.id}/run") {
            contentType(ContentType.Application.Json)
        }
